import { Card, Player, User } from '../../src/types/domain';
...
export class CardFactory {
   ...
   * カードを生成
   *
   * @param value カードの数字（A, 2-10, J, Q, K）
  ...
  static createMany(count: number): Card[] {
    ...
    for (let i = 0; i < count; i++) {
      cards.push({
        suit: suits[i % 4] as 'hearts' | 'diamonds' | 'clubs' | 'spades',
        value: values[i % 13],
        isPublic: false,
      });
    }
   ...
   *
   * @param values カード値の配列（例: ['A', '2', '3']）
   * @returns テスト用カード配列
   */
  static createWithValues(values: string[]): Card[] {
